...
| **Observed** | Over 120 instances at P = 6, D = 3: mean ratio **1.047** (SD 0.084), worst case **1.489**, and an optimal schedule found in **56.7 %** of instances. The plan never beat the reference optimum, which is the check that the Held-Karp implementation is correct. |
| **Observed (earlier)** | The same experiment read **1.229** (SD 0.162), worst case 1.665, optimal in only 8.3 % of instances before the improvement pass of §9.6 existed — greedy alone left roughly a quarter of the achievable makespan on the table. Adding relocation and 2-opt closed most of that gap: the construction is still greedy, but what is measured now is greedy *plus* local search. The figure is worth keeping because it shows what the improvement pass is worth, measured on the same instances rather than argued. Instances where a charging detour was needed are excluded, since the brute-force optimum models scheduling only; none arose at this size. |
